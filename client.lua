  ...
  if data then
    ...
    if itemName then
      ...
      if self.LearnOnCraft and not self.Learned[data] then 
        ...
        exports['mythic_notify']:DoCustomHudText('inform', 'You learned the recipe for: ' ..itemName , 2500) 
      end
...
function crafting:CraftRespond(response,label,items)
  if response then
    exports['mythic_notify']:DoCustomHudText('inform', 'You crafted: ' ..label , 2500)
  else
    exports['mythic_notify']:DoCustomHudText('inform', 'You failed to craft: ' ..label , 2500)
...
RegisterCommand('coords', function(source, args, raw)
  ...
  print(('Coords: %s\nHeading: %s'):format(GetEntityCoords(ped, true), GetEntityHeading(ped)))
end, false)